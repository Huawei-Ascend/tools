...
Result SampleProcess::Process(map<char, string>& params, vector<string>& input_files)
{
    ...
    {
        //如果输入的是目录，则对目录下面的文件进行遍历
        //如果是多输入路径，要保证多个目录下面的文件名一致
        ...
        for (int i=0;i<fileName_vec.size();++i)
        {
            ...
            processModel.OutputModelResult(output_path, modelName, 0);
        ...
        if (fileNums>0)
        {
            printf("Inference average time without first time: %.2f ms\n", total_time/(fileNums-1));
        }
    }
    
    else 
    {
        string framename = input_files[0];
        size_t dex = (framename).find_last_of(".");
        framename = (framename).erase(dex);
        dex = (framename).find_last_of("/");
        modelName = (framename).erase(0,dex+1);
        ...
        float infer_time_ave = Utils::InferenceTimeAverage(inference_time, loop);
        printf("Inference average time: %.2f ms\n", infer_time_ave);
        if (loop > 1) {
            float infer_time_ave_without_first = Utils::InferenceTimeAverageWithoutFirst(inference_time, loop);
            printf("Inference average time without first time: %.2f ms\n", infer_time_ave_without_first);
        }
        processModel.DestroyInput();
    }
